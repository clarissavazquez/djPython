...
def readNotes():
	...
	score = Score([])
	piano_staff = scoretools.StaffGroup([], context_name='MUSIC TEST')
	upper_staff = Staff([])
	piano_staff.append(upper_staff)
	score.append(piano_staff)
	upper_measure = []
	upper_measure.append(Measure((2, 4), []))
	upper_staff.extend(upper_measure)
	...
	for frequency in music:
		...
		print note
		note = printNotes(note)
		note = Note(note)
	music.close()
	show(note)

	#upper_measure[0].extend(note)
	#show(score)

def printNotes(frequency):
